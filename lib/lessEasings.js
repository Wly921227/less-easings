...
function lessEasings() {
  ...
  function _createVariablesList(obj) {
    ...
    var filename = 'lessEasingsVars.less',
        directory = './less',
        pathToLessEasingsVars = path.join(directory, filename),
        str = '';
    ...
    fs.writeFile(pathToLessEasingsVars, str, 'utf-8', function (err) {
      if (err) {
        console.log('Error: ' + err);
        return false;
      } else {
        console.log('File ' + pathToLessEasingsVars + ' was created!');
      }
    });
  }
